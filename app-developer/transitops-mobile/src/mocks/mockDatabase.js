...
export const MOCK_DB_STORAGE_KEY = '@transitops/mock-db-v1'
...
function createSeed() {
  ...
  return {
    ...
    trips,
    maintenanceLogs,
    // Alias — same array reference used by repositories
...
function applySnapshot(snapshot) {
  ...
  // Keep maintenance alias pointing at the same array as maintenanceLogs
  if (Array.isArray(next.maintenanceLogs)) {
    next.maintenance = next.maintenanceLogs
  } else if (Array.isArray(next.maintenance)) {
    next.maintenanceLogs = next.maintenance
  } else {
...
function toPersistable() {
  return {
    ...
    drivers: db.drivers,
    trips: db.trips,
    maintenanceLogs: db.maintenanceLogs || db.maintenance || [],
  ...
  try {
    ...
    if (raw) {
      ...
      await AsyncStorage.setItem(
        MOCK_DB_STORAGE_KEY,
        JSON.stringify(toPersistable()),
      )
    }
  } catch {
    // Keep in-memory seed if storage is unavailable
  } finally {
    hydrated = true
  }
}
...
/**
 * Load persisted mock DB (or seed) once. Safe to call repeatedly.
 */
export function ensureMockDbReady() {
  if (!readyPromise) {
    readyPromise = loadFromStorage()
  }
  return readyPromise
}
...
export async function persistDb() {
  ...
  try {
    ...
    // Ignore persistence failures in development
  }
}

/**
 * Shared in-memory mock database for the current app session.
...
/**
 * Clears AsyncStorage mock key and reseeds in-memory state.
 */
export async function resetDemoData() {
  try {
    await AsyncStorage.removeItem(MOCK_DB_STORAGE_KEY)
  } catch {
    // ignore
  }
  resetDb()
...
export function isMockDbHydrated() {
  return hydrated
}
...
export function upsertCredential({ userId, email, password }) {
  ...
  const existing = db.credentials.find(
    (item) => item.userId === userId || item.email === normalized,
  )
...
/**
 * Public user shape — never includes password / credential fields.
 */
